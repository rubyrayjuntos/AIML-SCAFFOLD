...
5. Run and review the Dev Terraform plan.
...
When native GitHub required reviewers are unavailable, R1 uses `manual_dispatch_with_plan_digest`: the plan workflow publishes a binary plan with source and generation provenance, and apply requires the exact reviewed digest, source commit, generation identity, target environment, and confirmation phrase. This is deliberate manual authorization, but it does not claim independent separation of duties when one person is the only operator.
